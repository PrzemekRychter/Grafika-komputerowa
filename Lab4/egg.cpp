...
void calculate(std::vector<std::vector<Point>> &points, int n){
    ...
    for(int i = 0; i < n; i++){
        ...
        for(int j = 0; j < n; j++){
            ...
            points[i][j].x =  (( -90*pow(u,5) + 225*pow(u,4) - 270*pow(u,3) + 180*pow(u,2) - 45*u) * cos(M_PI*v))/ 10.0f;
            points[i][j].y =  (160*pow(u,4) - 320*pow(u,3) + 160*pow(u,2)) /10.0f - 0.5f;      
            points[i][j].z =  (( -90*pow(u, 5) + 225* powf(u,4) - 270*powf(u, 3) + 180*powf(u,2) - 45*u) * sin(M_PI * v)) / 10.0f;
             float x_u = ((
                    -450*pow(u, 4)
                    +900*pow(u, 3)
                    -810*pow(u, 2)
            ...
            float length = sqrt(
               pow(normalVectorR[i][j].x, 2)
               +pow(normalVectorR[i][j].y, 2)
    ...
    if(model == 1){
       ...
       for (int i = 0; i< n-1; i++) {
            for (int j = 0; j < n-1; j++) {
                ...
                point3 p = { normalVectorR[i][j].x , normalVectorR[i][j].y , normalVectorR[i][j].z };
                glNormal3fv(p); 
                point3 p1 = { points[i][j].x  , points[i][j].y  , points[i][j].z } ;
                glVertex3fv(p1);
                point3 p44 = { normalVectorR[i][j+1].x , normalVectorR[i][j+1].y , normalVectorR[i][j+1].z  };
                glNormal3fv(p44);
                point3 p2 = { points[i][j+1].x  , points[i][j+1].y ,  points[i][j+1].z };
                glVertex3fv(p2);
                point3 p31 = { normalVectorR[i+1][j].x , normalVectorR[i+1][j].y , normalVectorR[i+1][j].z };
                glNormal3fv(p31);
                point3 p3 = { points[i+1][j].x , points[i+1][j].y  , points[i+1][j].z  };
                glVertex3fv(p3);
                glEnd();
                glBegin(GL_TRIANGLES);
                glNormal3fv(p31);
                glVertex3fv(p3);
                point3 pp = { normalVectorR[i+1][j+1].x , normalVectorR[i+1][j+1].y , normalVectorR[i+1][j+1].z };
                glNormal3fv(pp);
                point3 p4 = { points[i+1][j+1].x  , points[i+1][j+1].y,  points[i+1][j+1].z  };
                glVertex3fv(p4);
                glNormal3fv(p44);
                glVertex3fv(p2);
                glEnd();
...
int main(void){
    ...
    GLfloat mat_ambient[] = { 0.3, 0.0, 1.0, 1.0 };
    GLfloat mat_diffuse[] = { 0.0, 0.5, 0.6, 1.0 };
    GLfloat mat_specular[] = { 0.1, 0.2, 1.0, 1.0 };
